...
- **Los bots en producción (`DIFICULTADES` en `rooms.js`).** La dificultad es **la frecuencia de decisión**, no un handicap falso: no se le miente sobre el mundo ni se le rompe la física, sólo reacciona más tarde. No es un bot tonto, es un bot lento, que es exactamente en qué es peor una persona. Cinco niveles, medidos con 30 rondas cada uno: **Fácil 26,4 s (1,1 Hz) · Normal 44,4 (2,5) · Difícil 69,5 (5) · Imposible 111,1 (10) · Sobrehumano 126,6 (30)**. El espaciado no es lineal porque la supervivencia no lo es. **Los segundos no se muestran en la interfaz**: la política sigue mejorando y cualquier número absoluto envejece; el orden no.

- **Una afirmación mía que era falsa, y cómo se colaba.** Escribí —en un comentario del código, en un commit y acá— que decidir a 30 Hz hacía RENDIR PEOR a la política por quedar fuera de la distribución con la que aprendió. Era falso. Venía de una medición sobre **seis rondas por nivel**, y con vidas que van de 30 a 250 segundos seis muestras son ruido: vi "Imposible" en 108 s contra "Difícil" en 121 y lo leí como señal. Con 30 rondas, decidir más seguido **mejora** de forma clara (105,5 s a 10 Hz contra 130,4 a 30), porque la latencia de reacción baja de 100 a 33 ms y eso pesa más que el desajuste. Es exactamente la trampa de la muestra chica que este proyecto lleva documentada cuatro veces, y esta vez el que cayó fui yo escribiendo sobre no caer en ella. La lección práctica: **una conclusión no entra a un comentario de código hasta tener el tamaño de muestra anotado al lado.**

- **`maxBots` por nivel, y por qué.** El costo es lineal en las decisiones por segundo y el servidor es mono-hilo. Medido en el VPS: siete bots a 30 Hz piden unos **36 ms por tick contra un presupuesto de 33,3** — más que todo el presupuesto de una sala, y el mismo loop atiende `/agarra`, `/escapecv` y el leaderboard. Por eso "Sobrehumano" se topa en tres. 30 Hz es además el techo absoluto: no se puede decidir más seguido de lo que el mundo se actualiza.

- **La arena de Agarrá tickeaba para nadie.** Su `setInterval` corría los ocho forwards de 1,8 millones de parámetros treinta veces por segundo sin ningún guard, o sea **9,34 ms por tick —el 28% del presupuesto— las veinticuatro horas aunque no hubiera un solo socket conectado**. Se descubrió mirando el CPU del proceso en producción (77% de un core sostenido) cuando el usuario preguntó por el estado de una partida. Ahora la arena no simula si no hay público, y emite un último snapshot al vaciarse para que el que se va no quede con la pantalla congelada.
