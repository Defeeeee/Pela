...
const PUERTO = Number(process.env.PUERTO || 8420);
...
const VENTAJA_MINIMA = 1.15; // el nuevo tiene que ser 15% más grande
let actual = null;
let cambiadoEn = 0;
...
function elegirEspectador() {
  ...
  candidatos.sort((a, b) => b.mejorMasa - a.mejorMasa);
  ...
  if (ahora - cambiadoEn > ENFRIAMIENTO_MS &&
      mejor.fuente !== actual &&
      mejor.mejorMasa > vigente.mejorMasa * VENTAJA_MINIMA) {
    actual = mejor.fuente;
    cambiadoEn = ahora;
    return { ...mejor, cambio: true };
  }
